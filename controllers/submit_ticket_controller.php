...
$q = $db->fetchRow("SELECT `id` FROM ".TABLE_PREFIX."departments WHERE `autoassign_web` = 1 AND type=0");
if ($q){
	if (!in_array($action,array('displayForm','confirmation','confirmationMsg'))){
		$action = 'displayForm';
	}
	$department_id = intval($q['id']);
} elseif (isset($input->p['department'])) {
